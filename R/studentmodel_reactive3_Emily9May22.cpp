//9 May 2022 - Emily
//Using Ellen's new version of vaccination (adapted from original model) but combining within the relevant changes I made in late 2021 to include vaccination for the paper)
//new compartments, rates and ICs for vaccination
...
List c19uni(List params) {
 ...
 EV(0,j) = EV0[j];
  ...
  int flag=0,newcases=0,delta=0;//int flag=0,daycount=1,newcases=0,newcases_then=0,delta=0;
  double Etot=0.0;
    ...
    for(J = 0; J < nages; J++)
      {
        ...
        SE[J] = 1 - exp(-prob - backgroundrate*(1-0.5*(1-cos(2*M_PI*(t+200)/365))));
        REV[J] = 1 - exp(- (1.0 - ve_inf)*prob - (1.0 - ve_inf)*(backgroundrate*(1-0.5*(1-cos(2*M_PI*(t+200)/365)))));
      }
...
// R to S
  ...
  if(FLAG==1){printf("FLAG=%i\n",FLAG);break;}
    ...
    for(j = 0; j < nages; j++)
    {
      NN(t+1,j) = SS(t+1,j) + EE(t+1,j) + AA(t+1,j) + PP(t+1,j) + II(t+1,j) + RR(t+1,j) + HH(t+1,j) + QQ(t+1,j) + QA(t+1,j) + EV(t+1,j);

      if(isnan(NN(t+1,j)) || EE(t+1,j)<0)
      {
      printf("S=%f E=%f A=%f P=%f I=%f R=%f Q=%f H=%f EV=%f N=%f\n",SS(t+1,j),EE(t+1,j),AA(t+1,j),PP(t+1,j),II(t+1,j),RR(t+1,j),QQ(t+1,j),HH(t+1,j),EV(t+1, j), NN(t+1,j));
      break;
      }
    }
